...
class IconAdapter(var onClick: (IconModel) -> Unit) :
    BaseAdapter<IconModel, ItemEmojiBatteryEditBinding>() {
    ...
    override fun addListData(newList: MutableList<IconModel>) {
        listData.clear()
    ...
    override fun setData(
        binding: ItemEmojiBatteryEditBinding, item: IconModel, layoutPosition: Int
    ) {
        binding.apply {
            context?.let {
                viewLottie.visible()
                ...
                Glide.with(it).load(item.battery).listener(object : RequestListener<Drawable> {
                    override fun onLoadFailed(
                        e: GlideException?,
                        model: Any?,
                        target: Target<Drawable?>?,
